...
transactions = {
    ...
    'category': ['food & beverages', 'food & beverages', 'food & beverages', 'entertainment', 'others'],
...
expense_category_list = ['food & beverages', 'transportation', 'house needs', 'bills & utilities', 'pet', 'family', 'shopping', 'health & sports', 'donation', 'travel', 'entertainment', 'education', 'others']
income_category_list = ['salary', 'business', 'investment', 'gift', 'others']
                        ...
                        while transaction_type_input == 'expense' and transaction_amount_input > financialAccounts['balance'][financial_account_id]:
                            ...
                            while more_input not in ['yes', 'no']:
                                ...
                                if more_input == 'no':
                                    ...
                                    continue
'''
def view_transaction_page(user_id):
    if user_id not in transactions['user_id']:
        print('You haven\'t added any transaction.')
    else:
        while True:
            view_transaction_input = input(1. Original
2. Sorted by date
3. Filter by type (expense/income)
Please select an option from the menu above (1-3): )
            
            if view_transaction_input == '1':
                view_transactions(user_id=user_id)
                break
            elif view_transaction_input == '2':
                view_transactions(user_id=user_id, sorted_date=True)
                break
            elif view_transaction_input == '3':
                filter_type_input = input('Please enter filter by type (expense/income): ').lower()
                while filter_type_input not in ['expense', 'income']:
                    print('Invalid option. Please select an option between "expense" or "income".')
                    filter_type_input = input('Please enter filter by type (expense/income): ').lower()
                else:
                    view_transactions(user_id=user_id, filter_type=filter_type_input)
                    break
            else:
                print('Invalid option. Please select an option from the menu above (1-3).')'''
...
def view_budgets(user_id):
    if user_id not in budgets['user_id']:
        ...
        for i in b_user_id_index:
            ...
            print('| ID. | Date          | Type          | Category           | Amount        | Vendor        | Account Type  | Account Name  |')
            print('----------------------------------------------------------------------------------------------------------------------------')
            for y in budgets['transaction_id'][i]:
                ...
                while part_to_edit not in ['all', 'category', 'amount']:
                    ...
                    if part_to_edit == 'all':
                        ...
                        while (new_category_input - 1) not in range(0, len(expense_category_list)):
                            ...
                            confirm_input = input('Are you sure to edit the budget (Yes/No)? ').lower()
                            while confirm_input not in ['yes', 'no']:
                                print('Invalid option. Please select between "Yes" or "No".')
                                confirm_input = input('Are you sure to edit the budget (Yes/No)? ').lower()
                            else:
                                if confirm_input == 'yes':
                                    budgets['category'][budgets['id'].index(edit_budget_id)] = expense_category_list[new_category_input - 1]
                        ...
                        new_amount_input = input('Enter new amount: ')
                        confirm_input = input('Are you sure to edit the budget (yes/no)? ').lower()
                        while confirm_input not in ['yes', 'no']:
                            print('Invalid option. Please select between "yes" or "no".')
                            confirm_input = input('Are you sure to edit the budget (yes/no)? ').lower()
